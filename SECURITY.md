...
### 🔴 v1 の ecash は実 mint では通らない — 既定で接続を拒否する (2026-08-18)

**転送の平文問題と対になる、金銭側の開示である。**

- `core::ecash::build_proof` と `net::cashu_mint::build_blinded_outputs` は
  **BDHKE のプレースホルダ**。`k256` をこの環境に追加できず、
  §1.1 が楕円曲線演算の手書きを禁じているため。
- この状態で**実 mint に Lightning 入金すると、unblind できない proof しか
  返らない — 入金した sats は取り戻せない**。
  転送の平文はプロンプトが漏れるが、こちらは**資金が消える**。
- **したがって `CashuClient::new` はループバック以外の mint を既定で拒否する。**
  試すには `ROPE_ALLOW_PLACEHOLDER_ECASH=1` を明示する必要がある。
  拒否時のメッセージは理由と外し方の両方を示す。
- ループバック判定は **URL 文字列で**行う。DNS を引くと
  `localhost.evil.com` のような名前で実 mint へ向けられる余地が残るため。

### 🔴 v1 のジョブ転送は暗号化されていない (2026-08-18 実装)

**これは v1 で最も重要な開示である。**

`src/net/transport.rs` は実際に TCP でジョブを運ぶが、**中身は平文**である。

- **提供するもの**: 完全性と真正性。全フレームが実 `ed25519-dalek` で署名され、
  受信側は `verify_strict` で検証する。**第三者はジョブや結果を書き換えられない**し、
  なりすまして送ることもできない。
- **提供しないもの**: **秘匿性**。同じ LAN の受動的な盗聴者は
  **プロンプトも生成結果も読める**。
- **なぜこうなっているか**: Noise 鍵交換には検証済みの暗号 crate (`snow` /
  `x25519-dalek`) が要るが、この環境では追加できない。そして
  [`docs/SURPLUS_AND_GAPS.md`](docs/SURPLUS_AND_GAPS.md) §1.1 は
  **「検証済み crate 無しに Noise を手書きするな」**と定めている。手書きはしない。
- **既定で無効。** 転送を使うには `ROPE_ALLOW_PLAINTEXT=1` を明示的に立てる必要が
  ある。立てなければ `rope earn` は待ち受けず、`rope run` はピアに頼まず、
  理由と有効化方法を表示してローカル実行へ落ちる。
- **どういう時なら妥当か**: 自宅・研究室など**LAN 上の全員を信頼できる**場合に限る。
  カフェや共有オフィスの Wi-Fi では有効化しないこと。

⚠️ **鍵の真正性についても過信しないこと。** 署名検証が証明するのは
「相手はその秘密鍵を持っている」ことだけで、**その鍵が意図した相手のものか**は
証明しない。v1 の判断材料は**確認コードの目視照合 (TOFU)** のみである。

### v1 の LAN 発見が晒すもの / 晒さないもの (2026-08-18 実装)

`src/net/mdns.rs` は**平文の UDP マルチキャスト**である。これは欠陥ではなく
mDNS そのものの性質で、AirDrop・Chromecast・ネットワークプリンタと同じ。

- **晒すもの**: 「このホストが Rope を動かしている」という事実、インスタンス名、
  ノード ID、**公開鍵と指紋** (いずれも定義上公開情報)。
- **晒さないもの**: プロンプト・ジョブ・決済。**これらはネットワークを一切
  渡らない** — 転送層 (Noise) が未実装だからである。
- **受信パケットは全て敵性入力として扱う**: LAN の誰でも任意の mDNS を送れる。
  パーサは境界検査付きで、名前圧縮ポインタのループを打ち切り、レコード数・
  名前長に上限を課し、解釈できないレコードは捨てる。ランダム入力と全切り詰めで
  panic しないことをテストしている。
- ⚠️ **発見されたピアの名乗りは検証されていない。** 誰でも任意の公開鍵と名前を
  名乗れる。**確認コードの目視照合 (TOFU) が唯一の防御**であり、v1 ではそれ以上を
  提供しない。
