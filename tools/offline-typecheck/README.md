...
tools/offline-typecheck/run-tests.sh      # テスト 318 件 <!-- tests:http --> を**実際に実行**
...
**`src/` のテスト 318 件 <!-- tests:http --> すべてがこの環境で実際に走る**
(2026-08-18〜)。内訳は 310 件 <!-- tests:default --> が既定 feature、
残りが `--features http`。依存ゼロの `src/net/` だけを単独ビルドした
67 件 <!-- tests:net --> は前者の部分集合である — **3 つを足してはいけない。**
