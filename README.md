...
> - 🔶 **ピア発見は本物** (2026-08-18) — 同じ LAN の Rope を実際に見つける。
>   ただし **Noise 鍵交換とジョブ転送が無いので、見つかっても話せない**。
> - ❌ **まだ「他人の」ではない。** 転送と ecash 決済が未結線のため、
>   計算は**自分のマシンで**走る。
...
> - **ピア探索は本物になった** (2026-08-18)。`rope pair` は実 UDP マルチキャストで
>   `_rope._tcp.local` を探索し、同じ LAN の Rope を実際に見つける
>   (`src/net/mdns.rs`)。ただし初回デモの「📡 近くのピアを探しています」の行は
>   まだ演出のまま。
> - **接続は無い。** Noise 鍵交換もジョブ転送も未実装で、**ピアは見つかるが
>   話せない**。つまり計算は走っても、**まだ「他人の」GPU では走らない**。
> - **GPU ではなく CPU** で走る。
>
...
|---|---|
| LAN 発見 (mDNS) — 🔶 **発見は実装済**、転送は未実装 ・TOFU 信頼 | **TEE 秘匿 (A6)** — 消費者 GPU に CC が無く (W4)、構造的緊張 4 件中 3 件の源 |
