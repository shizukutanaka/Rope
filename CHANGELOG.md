...
- **🔴 slow-loris を塞いだ — そして「射程を超える」と書いた自分の判断が
  誤りだったことを記録する** (`SURPLUS_AND_GAPS.md` §1.21 の続き)。
  **型検査 PASS + ハーネスでテスト実行 PASS (318 件)。cargo は未検証。**
  - §1.21 で見つけた時、私は「塞ぐには接続全体の締切が要り、**v1 の射程を
    超える**」と書いて残し、`CLAUDE.md` の短所 W10 に載せた。
    ソクラテス問答「**その根拠は何か。実際に何行かかるのか数えたか?**」で
    **数えていなかった**ことが判明
  - 必要なものは全部あった: `read_frame_bytes` は `&mut impl Read` を取るので
    **`Read` を実装したラッパを 1 つ挟むだけ**。`TcpStream` の
    `set_read_timeout` に「締切までの残り」を毎回張り直せば、**合計時間が
    締切そのもので抑えられる**。**新しい crate は不要。約 30 行**
  - `DeadlineReader` を新設し、貸し手の読み出し 3 箇所を包んだ。
    **借り手側は意図的に包まない** — 貸し手の計算完了を待つので最初の
    1 バイトまでの時間が原理的に読めず、締切を張ると遅いモデルで正当な
    依頼が切れる。**この非対称は漏れではなく設計判断**
  - **実測**: 滴下する相手に対し **15.005 秒 → 5.70 秒**。同じ変異で
    `a_silent_borrower_...` は通ったままで、**滴下が黙秘とは別の穴**だと
    示せた
  - **テストが「正しい理由で」通っているかも疑った**: 最初の版は
    ヘッダの版番号で 5.5 秒に弾かれており、**締切が効いていなくても通る**
    テストだった。本物の `Hello` フレームを 1 バイトずつ送る形に直した。
    **新しいテストは、壊した実装で落ちることを確かめるまで信用しない**
  - `CLAUDE.md` の W10 を削除 (直ったので)

### Changed

- **`CLAUDE.md` を 6 ラウンドの結果に合わせて更新** — 次のセッションが最初に
  読む文書なので、ここが古いと全部が古くなる。
  - 冒頭の「実態」が「**まだネットワーク越しには動かない**」のままだった —
    同じ項目の箇条書きは転送が動くと書いており、**1 項目の中で矛盾**していた
  - W1「実 I/O が皆無」は 2026-08-18 に解消済なのに残っていた。残るのは
    BDHKE と Noise の 2 つだけ、と書き直した
  - W4「v1 では A6 ごと削除して解消」に**取りこぼしがあった** (§1.24) ことを
    追記。「削除した」と書いた後に経路へ残っていないか確かめる、という
    教訓ごと残す
  - W9 (SwiGLU の照合が無い) / W10 (slow-loris が通る) を追加 — 今回の
    修正で**残ったもの**を短所として明示する
  - 改善案表に「2026-09-01: ソクラテス問答 6 ラウンド」の一覧を追加。
    **6 件はどれも「実装が足りない」ではなく「主張と実態がずれている」**
    だった、という教訓を次のセッション向けに書いた
